...
export default function ProfileTypeForm({ onChange }: IProfileType) {
  const profileType = useRef<'mentor' | 'aluno' | null>(null);

  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!profileType.current) return;
    onChange?.(profileType.current as string);
  };

  return (
    <div className="max-w-md h-full flex flex-col space-y-3 items-center sm:items-start mt-4">
      ...
      <form className="flex flex-col gap-3 mt-3" onSubmit={handleSubmit}>
        <CardChooseProfile
          ...
          onChange={() => (profileType.current = 'mentor')}
        />
        <CardChooseProfile
          ...
          onChange={() => (profileType.current = 'aluno')}
        />
